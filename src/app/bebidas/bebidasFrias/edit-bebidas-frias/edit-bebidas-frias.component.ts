...
@Component({
  selector: 'app-edit-bebidas-frias',
  templateUrl: './edit-bebidas-frias.component.html',
  styleUrls: ['./edit-bebidas-frias.component.css']
})
export class EditBebidasFriasComponent {
  cantidad: any; 
  precio:number = 0;
  nombre: string = '';
  Orden:any;
  ...
  bebidas:any = [];
  constructor(private service: BebidasFriasService, private add: CarritoService, private router: Router, private alertService: AlertDialogService, private activatedRoute: ActivatedRoute, private authService: AuthService){}
  ngOnInit() {
    ...
    this.service.getBebidas().pipe(
      switchMap((res: any) => {
        ...
        // Ahora obtenemos la orden solo cuando las bebidas están listas
